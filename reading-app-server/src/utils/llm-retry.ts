export interface LLMErrorClassification {
  retryable: boolean;
  provider: 'google' | 'openai' | 'anthropic' | 'unknown';
  reason:
    | 'rate_limit'
    | 'service_unavailable'
    | 'timeout'
...
export interface RetryLLMOperationOptions<T> {
  operation: () => Promise<T>;
  maxRetries: number;
...
export function retryDelayMsForLLMError(
  classification: LLMErrorClassification,
  attempt: number,
  defaultDelayMs: number,
  maxDelayMs: number,
...
export async function retryLLMOperation<T>(options: RetryLLMOperationOptions<T>): Promise<T> {
  let attempt = 0;
  ...
  while (true) {
    try {
      ...
      const delayMs = retryDelayMsForLLMError(
        classification,
        attempt,
...
function detectProvider(message: string): LLMErrorClassification['provider'] {
