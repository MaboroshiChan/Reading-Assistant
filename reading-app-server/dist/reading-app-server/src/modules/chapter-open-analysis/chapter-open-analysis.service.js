...
let ChapterOpenAnalysisService = class ChapterOpenAnalysisService {
    ...
    constructor(bookIngestionRepository, chapterOpenAnalysisRepository, chapterKeywordsWorkflowService, chapterKeywordsWorkflowRepository, knowledgeExtractionWorkflowService, knowledgeExtractionWorkflowRepository, quizWorkflowRepository, workflowQueueService) {
    ...
    reconcileRun(run) {
        ...
        const chapterKeywordsTask = this.taskFromChildRun(chapterKeywordsRun?.status, chapterKeywordsRun?.id, chapterKeywordsRun?.error);
        const knowledgeTask = this.taskFromChildRun(knowledgeRun?.status, knowledgeRun?.id, knowledgeRun?.error);
        ...
        const hasTerminalFailure = [chapterKeywordsTask, knowledgeTask, quizTask]
        ...
        const aggregateStatus = (() => {
            ...
            if (chapterKeywordsTask.status === 'running'
                || chapterKeywordsTask.status === 'queued'
                || knowledgeTask.status === 'running'
                || knowledgeTask.status === 'queued'
                || quizTask.status === 'blocked'
                || quizTask.status === 'queued'
                || quizTask.status === 'running') {
                return 'running';
            }
